<?php
	...
	if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET["eventId"]) === true) {
		try {
			$_POST = EventModule::loadEvent($_GET["eventId"], $ownAccessLevel);
...
<html lang="de">
	<head>
		...
		<script src="<?php echo(ROOT_PUBLIC);?>/js/events_edit.js"></script>
	...
	<body>
		...
		<div class="container">
			...
			<div class="jumbotron jma-background-color">
				...
				<hr>
				<h3 class="float-left"><?php echo($GLOBALS["dict"]["event_overview"]);?></h3>
				<a href="events.php" class="btn btn-primary float-right ml-2"><?php echo($GLOBALS["dict"]["event_list_back"]);?></a>
				<div class="clearfix"></div>
				<hr>
				<form name="eventEditForm" method="POST" class="form-horizontal">
					<div class="form-group">
						<label class="control-label col-12" for="eventId"><?php echo($GLOBALS["dict"]["event_eventId"]);?></label>
						<div class="col-12 input-group">
							<span class="input-group-addon"><i class="fas fa-list-ol fa-fw"></i></span>
							<input name="eventId" type="text" class="form-control"
								value="<?php if (isset($_GET["eventId"]) === true) echo($_GET["eventId"]);?>" disabled>
						</div>
					</div>
					<div class="form-group">
					...
					<hr>
					<a href="#" class="btn btn-warning mx-2" data-toggle="modal" data-target="#announceEventModal"
						data-eventid="<?php echo($_POST["eventId"]);?>" data-title="<?php echo(htmlspecialchars($_POST["eventTitle"]));?>" 
						data-requiredaccessidentifier="<?php echo($_POST["requiredAccessIdentifier"]);?>"><?php echo($GLOBALS["dict"]["label_announce"]);?></a>
					<a href="images.php?eventId=<?php echo($_GET["eventId"]);?>" class="btn btn-info"><?php echo($GLOBALS["dict"]["image_manage_linked"]);?></a>
					<a href="events_participants.php?eventId=<?php echo($_GET["eventId"]);?>" class="btn btn-success"><?php echo($GLOBALS["dict"]["event_participants_list"]);?></a>
					<hr>
					<p>
						<?php echo($GLOBALS["dict"]["label_update_last"]); 
						if(isset($_POST["eventModificationDate"]) === true) { 
							echo($_POST["eventModificationDate"]); 
						} else { 
							echo("-");
						}?>
					</p>
		...
		<div class="modal fade" id="announceEventModal" tabindex="-1">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
					...
					<div class="modal-body">
						<p><?php echo($GLOBALS["dict"]["pn_event_send_modal_safeguard_1_prefix"]);?><strong><?php echo(htmlspecialchars($_POST["eventTitle"]));?></strong><?php echo($GLOBALS["dict"]["pn_event_send_modal_safeguard_1_suffix"]);?></p>
